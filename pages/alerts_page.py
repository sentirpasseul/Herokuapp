    ...
    def check_js_prompt(self):
        try:
            ...
            if AlertsData.PROMPT_TEXT == self.browser.get_alert_text():
                self.browser.send_keys(AlertsData.PROMPT_TEST_TEXT)
                self.browser.confirm_alert()
                if self.get_result_element_text() == (AlertsData.PROMPT_RESULT_TEXT + AlertsData.PROMPT_TEST_TEXT):
                    Logger.info(f"Check JavaScript prompt successful: {self.get_result_element_text()}"
                                f"== {(AlertsData.PROMPT_RESULT_TEXT + AlertsData.PROMPT_TEST_TEXT)}")
                    return Tru
        except Exception as err:
